...
def photo_and_geometry_loss(tgt_img, ref_imgs, tgt_depth, ref_depths, intrinsics, poses, poses_inv, hparams):
    ...
    diff_img = torch.cat(diff_img_list, dim=1)  # 光度重建误差
    diff_color = torch.cat(diff_color_list, dim=1)
    diff_depth = torch.cat(diff_depth_list, dim=1)  # 深度重建误差
...
def compute_pairwise_loss(tgt_img, ref_img, tgt_depth, ref_depth, pose, intrinsic, hparams):
    ...
    source_img = tensor2numpy(tgt_img)  # tgt_img torch.Size([4, 3, 256, 832])
    target_img = tensor2numpy(ref_img_warped)
    frame = source_img
    for i in range(2):
    ...
    # tensor可视化
    vis = visualize_depth(mask_img[0, 0]).permute(1, 2, 0).numpy() * 255 # valid_mask[0, 0]).permute(1, 2, 0)
    imwrite('demo/mog_test/{}.jpg'.format('test_temp'),vis.astype(np.uint8))

    return diff_img, diff_color, diff_depth, valid_mask


# compute mean value on a binary mask 计算二进制掩码的平均值
