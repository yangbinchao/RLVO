...
# Describe: Unet3p 模型代码，thanks for https://www.github.com/avBuffer/UNet3plus_pth
            深度估计，输出5个尺度的深度图
'''
...
class UNet3P_m(nn.Module):
    def __init__(self, n_channels=3, n_classes=1, alpha=10, beta=0.01, bilinear=True, feature_scale=4,
                 is_deconv=True, is_batchnorm=True):
        super(UNet3P, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes
        ...
        # DeepSup
        self.outconv1 = nn.Conv2d(self.UpChannels, n_classes, 3, padding=1)  # 320 -> 1
        ...
        #             nn.Dropout(p=0.5),
        #             nn.Conv2d(filters[4], 2, 1),  # input: 20*20*1024 -> output: 20*20*2
        #             nn.AdaptiveMaxPool2d(1),    # input: 20*20*2 -> output: 1*1*2
        #             nn.Sigmoid())
    ...
    # def dotProduct(self,seg,cls):  # cls input: B*1(B*channel)
    #     B, N, H, W = seg.size()
    #     seg = seg.view(B, N, H * W)
    #     final = torch.einsum("ijk,ij->ijk", [seg, cls])  # 爱因斯坦求和约定（einsum）
    #     # 箭头左边表示输入张量，以逗号分割每个输入张量，箭头右边则表示输出张量。表示维度的字符只能是26个英文字母 'a' - 'z'。
    #     final = final.view(B, N, H, W)
    #     return final

    def predict_disp(self,d):
        sg = nn.Sigmoid()
        return sg(d)
    ...
    def forward(self, inputs):
        ## -------------Encoder-------------
        h1 = self.conv1(inputs)  # h1->320*320*64  input: 640*640*3 -> output: 320*320*64
        ...
        h5 = self.maxpool4(h4)
        hd5 = self.conv5(h5)  # h5->20*20*1024  input: 40*40*512 -> output: 20*20*1024
        
        '''
        # -------------Classification-------------
        cls_branch = self.cls(hd5).squeeze(3).squeeze(2)  # (B,N,1,1)->(B,N)  B*2*1*1
        cls_branch_max = cls_branch.argmax(dim=1)  # channel维度判断类别
        cls_branch_max = cls_branch_max[:, np.newaxis].float()  # np.newaxis的作用就是在这一位置增加一个一维
        '''        
        ...
        # d5 = self.outconv5(hd5)  # input: 20*20*1024 -> output: 20*20*1
        # d5 = self.upscore5(d5) # 16->256  16倍上采样 320*320

        # d4 = self.outconv4(hd4)  # input: 40*40*320 -> output: 40*40*1
        # d4 = self.upscore4(d4) # 32->256  8倍上采样  320*320
        ...
        disp1 = self.alpha * self.predict_disp(d1) + self.beta
        # disp2 = self.alpha * self.predict_disp(d2) + self.beta
        # disp3 = self.alpha * self.predict_disp(d3) + self.beta
        # disp4 = self.alpha * self.predict_disp(d4) + self.beta
        # disp5 = self.alpha * self.predict_disp(d5) + self.beta
...
if __name__ == "__main__":
    ...
    model = UNet3P().cuda()
    model.train()
    test_data = torch.rand(2, 3, 160, 512).cuda() # 256, 832
...
if __name__ == '__main__':
    ...
    model = UNet3P().to(device)
    summary(model, (3, 256, 832))  #   416, 128
'''